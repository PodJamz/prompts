...
interface InteractionApproach {
  introduction: string;
  firstPrompt: string;
  reassurance: string;
}

interface DepthApproaches {
  direct: string;
  metaphorical: string;
  supportive: string;
}

interface ClosingMessages {
  reflection: string;
  invitation: string;
}
...
interface Instructions {
  ...
  exampleInteractionFlow: InteractionStep[];
  neverDoThis: string[];
  interactionSequence: InteractionSequence;
  adaptationGuidelines: Record<string, string>;
}
...
interface HydraResponse {
  ...
  response: {
    ...
    context?: {
      personality: string;
      directives: string[];
      currentPhase?: 'invitation' | 'surfaceLayer' | 'depth' | 'reflection' | 'integration' | 'closing';
    };
  };
}

interface HydraSessionUpdate {
  type: 'session.update';
  session: {
    context: string;
  };
